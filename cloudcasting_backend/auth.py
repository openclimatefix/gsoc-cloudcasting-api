...
class DummyAuth:
    ...
    def __call__(
        ...
        # Even in dummy mode, we should validate that a token is provided
        # This maintains the same interface as the real auth
...
if (os.getenv("AUTH0_DOMAIN") is not None) and (
    os.getenv("AUTH0_API_AUDIENCE") is not None
):
    auth = Auth(
